...
//
// The behaviour under test is InteractiveSessionCookieEvents.RedirectToLogin,
// which forces the redirect back on for the browser-driven OpenIddict endpoints
// and defers to base for everything else. Note that the override, not
// ConfigureApplicationCookie, is the only place this can live: the application
// cookie sets EventsType, and a handler resolves events from EventsType *or*
// Options.Events, never both — so assigning options.Events.OnRedirectToLogin
